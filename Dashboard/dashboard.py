...
if 'review_score' in reviews_df.columns and 'review_comment_message' in reviews_df.columns:
    review_score_count = reviews_df['review_score'].value_counts().sort_index()
    ...
    all_comments = " ".join(reviews_df['review_comment_message'].dropna().astype(str).tolist())
...
# Gabungkan ke orders
merged_df = orders_df.merge(products_df[['product_id', 'product_category_name_english']], on='product_id', how='left')

if 'product_category_name_english' in merged_df.columns:
    kategori_unik = merged_df['product_category_name_english'].dropna().unique()
    selected_kategori = st.sidebar.multiselect("Pilih Kategori Produk", sorted(kategori_unik))
    if selected_kategori:
        merged_df = merged_df[merged_df['product_category_name_english'].isin(selected_kategori)]

st.subheader(" Top 10 Kategori Produk Terlaris")
top_kategori = merged_df['product_category_name_english'].value_counts().head(10)
